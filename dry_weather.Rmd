...
hotdat <- reactive({
  ...
  out <- out %>% 
    ...
    summarise(
      n = unique(n),
      exceeds_count = sum(abv, na.rm = T),
      unexceeds_count = n - exceeds_count,
      ) %>% 
    ungroup() %>% 
    mutate(
      exceeds_pct = round((exceeds_count / n) * 100, 0),
      unexceeds_pct = (100 - exceeds_pct),
      exceeds = exceeds_pct,
      unexceeds = unexceeds_pct,
      cols2 = hotcol(unexceeds_pct),
      cols = hotcol(exceeds_pct), 
      cexs = scales::rescale(exceeds_pct, to = c(4, 17), from = c(0, 100))
    ) %>% 
    # exceeds and unexceeds are left in for compatibility with other code Emily may have written, but they essentially are the exceed/unexceed %
    select(Watershed, StationCode, n, exceeds_count, exceeds_pct, unexceeds_count, unexceeds_pct, cols, cols2, cexs, exceeds, unexceeds) %>% 
...
hotplo2 <- eventReactive(list(input$go_quality),{
  ...
  toplo_exceeds<-toplo %>%
    select(Watershed, StationCode, exceeds_count, exceeds_pct, n, cols)%>%
    rename(exceedences = exceeds_count, exceedences_pct = exceeds_pct)%>%
    mutate( Name = "exceeds", nameforlabel = "exceeding threshold")
  
  toplo_unexceeds<-toplo%>%
    select(Watershed, StationCode, unexceeds_count, unexceeds_pct, n, cols2)%>%
    rename(exceedences = unexceeds_count, exceedences_pct = unexceeds_pct)%>%
    rename(cols = cols2)%>%
    mutate( Name = "unexceeds", nameforlabel = "below threshold")
  
  #toplo<-full_join(toplo_exceeds, toplo_unexceeds)
  toplo<-bind_rows(toplo_exceeds, toplo_unexceeds) %>% 
    rename(exceedences_count = exceedences)

  print(toplo)
  
  Exceedences<-paste0(
    "Station ID: ", toplo$StationCode, '\n',
    "Constituent: ", input$varsel3, '\n',
    toplo$n," sample(s)", '\n',
    toplo$exceedences_count," sample(s) ",toplo$nameforlabel, '\n',
    toplo$exceedences_pct,"% of samples ",toplo$nameforlabel, '\n'
  )
#Barplot
  
  toplo %>% write.csv('ignore/toplo.csv')
  
  p <- ggplot(toplo, aes(x = StationCode, y = exceedences_count,label= Exceedences, text = Exceedences)) + #, fill=toplo$cols)) +
...
"Enter your data in the downloadable Excel Template OR downloaded data from CEDEN using this link (https://ceden.waterboards.ca.gov/AdvancedQueryTool) for upload into the app. The app will accommodate up to 35,000 rows of data. ",br(),
...
"5. If you have multiple constituents in your data, select the one you want to visualize",br(),
...
box(title= "Box Plots of Constituent Concentrations by Watershed and Station", status = "warning", collapsed = TRUE,collapsible = TRUE, width = 12, solidHeader = TRUE,
fluidRow(
  p(
...
box(title= " Bar Plots of Constituent Sample Size by Watershed and Station", status = "warning", collapsed = TRUE,collapsible = TRUE, width = 12, solidHeader = TRUE,
fluidRow(
  p(
